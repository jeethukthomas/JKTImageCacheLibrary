...
{
    if(imgUrl.length>0 && userID)
    {
     NSString* url=[NSString stringWithFormat:@"%@%@userID=%@&fileUrl=%@&signature=%@",k_url_baseURLString,k_url_downloadImage,userID,imgUrl,[[IfynderHelper sharedHelper] getUserSignature] ];
    ...
    else
    {
        ...
        [self getImage:url withFilename:filename completed:^(NSString *path) {
            [self stopRotating:imgView];
